...
Task 1: Start with an initial nuumber(e.g. 10).
...
def multiply_by_two(**context):
    ...
    context["ti"].xcom_push(key='current_value',value=new_value)
    print(f"Multiple by 2:{current_val} * 2={new_value}")

def subtract_three(**context):
    current_val=context['ti'].xcom_pull(key='current_value',task_ids='multiply_by_two_task')
    new_value=current_val-3
    context["ti"].xcom_push(key='current_value',value=new_value)
    print(f"Subtract by 3:{current_val} - 3={new_value}")  
